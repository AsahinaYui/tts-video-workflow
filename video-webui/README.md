...
脚本会自动打开浏览器。浏览器页面真正关闭后，WebUI 会在短暂 unload 延迟后自动结束后台服务；后台标签页漏掉定时心跳不会触发退出。
...
索引建立会记录每个编号的时间轴和对齐置信度，并把相邻小句边界吸附到附近的静音谷。若某段置信度很低，或坏音频已经跨过边界，例如 `4.1` 吞掉了 `3.2` 的尾音，建议直接输入连续编号 `3.2,4.1` 一起替换。

应用替换时会使用采样级 `atrim` 裁切、相邻段保护窗口和短交叉淡化。实际裁掉的起止点会写入 `logs/sentence_patch_direct_splice/splice_boundaries.json`，排查接缝时优先看这个文件。

音频修补后会重新按新音频更新时间轴，但默认保留 SRT 校对编辑器里的字幕分页和手动换行。最终渲染传入现成 SRT 时也会保留 SRT 的行布局，不再二次重排。

SRT 文件只可靠保存字幕序号、时间轴和文本换行；字体、字号、描边、边距等样式由渲染脚本的 ffmpeg `force_style` 控制，不属于标准 SRT 样式信息。
